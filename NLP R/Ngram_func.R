loadNGram <- function(Corpus, N){
    ntokenizer <- function(x, tokens) function(x) NGramTokenizer(x, RWeka::Weka_control(min = tokens, max = tokens))
...
dfConvert <- function(tdm, N){ # N arg only for id of token length to purge hapax
