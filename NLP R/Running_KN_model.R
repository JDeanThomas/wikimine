...
# setup wd and read in file here
...
# viable alternative to tm Package? 
# better way to read in and matrix of test, validate and test sets?

#############    train, test validation   ########################
# sequential cut (statistical cut would be better)
partitionData <- function(datadir, fileb, probcut=c(0.6, 0.8, 1) ){  #don't need "probcut"
    file <- paste(datadir, fileb, sep="\\")
...
# Probably use TM for this instead
...
loadNGram <- function(corpus, N){
    tokenizer <- function(x, toks) function(x) NGramTokenizer(x, Weka_control(min = toks, max = toks))
    tdm <-   TermDocumentMatrix(corpus, control = list(tokenize = tokenizer(toks=N),wordLengths=c(1,Inf)))
    df <- dfConvert(tdm, N)
    return(df)
}

# function to create data.frames from Term Document Matrices 

dfConvert <- function(tdm, N){
    counts <- rowapply_simple_triplet_matrix(tdm, sum)
    tdmDT <- data.table(term = Terms(tdm), cnt = counts)
    setkey(tdmDT, term)
    # purge hapax (keep always unigram hapax)
    if(NOHAPAX==T&N>1){
        tdmDT <- tdmDT[tdmDT$cnt>1,]
    }
    rm(counts)
    return(tdmDT)
}
